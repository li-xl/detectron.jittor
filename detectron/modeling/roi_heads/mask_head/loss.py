...
def project_masks_on_boxes(segmentation_masks, proposals, discretization_size,maskiou_on):
    ...
    for segmentation_mask, proposal in zip(segmentation_masks, proposals):
        ...
        if maskiou_on:
            x1 = int(proposal[0].item())
            y1 = int(proposal[1].item())
            x2 = int(proposal[2].item()) + 1
            y2 = int(proposal[3].item()) + 1
            # for poly_ in segmentation_mask.polygons:
            for polygonInstance_ in segmentation_mask.instances.polygons:
                poly = polygonInstance_.polygons[0].numpy()
            ...
            full_area = jt.array(mask_util.area(rle_for_fullarea).sum().astype(np.float32))
            rle_for_box_area = mask_util.frPyObjects([p.polygons[0].numpy() for p in cropped_mask.instances.polygons], (proposal[3]-proposal[1]).item(), (proposal[2]-proposal[0]).item())
            box_area = jt.array(mask_util.area(rle_for_box_area).sum().astype(np.float32))
            mask_ratio = box_area / full_area            
            mask_ratios.append(mask_ratio)

    if len(masks) == 0:
        return jt.zeros((0,)).float32(),jt.zeros((0,)).float32()
    if maskiou_on:
        mask_ratios = jt.contrib.concat(mask_ratios, dim=0).float32()
    else:
        mask_ratios = None

    return jt.stack(masks, dim=0).float32(),mask_ratios
...
class MaskRCNNLossComputation(object):
    ...
    def __call__(self, proposals, mask_logits, targets):
        ...
        if self.maskiou_on:
            ...
            mask_targets_full_area = mask_targets.sum(dims=[1,2]) / mask_ratios
            mask_ovr = pred_masks * mask_targets
            mask_ovr_area = mask_ovr.sum(dims=[1,2])
            mask_union_area = pred_masks.sum(dims=[1,2]) + mask_targets_full_area - mask_ovr_area
